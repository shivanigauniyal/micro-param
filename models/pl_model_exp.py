...
class LightningModel(pl.LightningModule):
    ...
    def loss_function(self, updates, y, k=None):
        ...
        if self.loss_absolute:
            if self.step_size > 1:
                pred_loss = (
                    self.criterion(self.pred_moment_norm[:, 0], y[:, 0])
                    + self.criterion(self.pred_moment_norm[:, 1], y[:, 1])
                    + self.criterion(self.pred_moment_norm[:, 2], y[:, 2])
                    + (0.005 * self.criterion(self.pred_moment_norm[:, 3], y[:, 3]))
                ) / 4
            else:
